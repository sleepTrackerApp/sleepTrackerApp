...
const getDailyInsight = async (req, res) => {
  try {
    ...
    const latestLogUpdate = Math.max(...sleepEntries.map(e => new Date(e.updatedAt).getTime()));
    ...
    const existingInsight = await AIInsight.findOne({ userId, dayKey, endDate: latestLogDate });

    if (existingInsight) {
      const insightGeneratedAt = new Date(existingInsight.generatedAt).getTime();
      // If the report was made AFTER the last log update, serve the cache
      if (insightGeneratedAt > latestLogUpdate && existingInsight.analysis.includes(goalText)) {
    ...
    console.log('[AI Scientist] Change detected in logs or goal. Generating fresh insight...');
    ...
    const updatedRecord = await AIInsight.findOneAndUpdate(
